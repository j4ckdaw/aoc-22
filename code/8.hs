...
createGrid input = array ((0,0), (h,w)) [ ((i,j), getTree (i,j)) | (i,j) <- range ((0,0), (h,w)) ]
    where
        w = length (input !! 0) - 1
        h = length input - 1
...
solveA :: [String] -> Int
solveA input = length $ filter id [ visible grid (i,j) | (i,j) <- range (bounds grid) ]
    where grid = createGrid input

solveB :: [String] -> Int
solveB input = maximum [ scenicScore grid (i,j) | (i,j) <- range (bounds grid) ]
    where grid = createGrid input
